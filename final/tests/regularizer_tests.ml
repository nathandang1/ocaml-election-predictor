...
let test_concatenate = [
  ( "concatenate_lists test (positive x positive)" >:: fun _ ->
    assert_equal (Regularizer.concatenate_lists lst1 lst2) 
    [ (1.5, 0.5); (3.2, 2.2); (4.3, 5.4) ] ); 
  ( "concatenate_lists test (positive x negative)" >:: fun _ ->
    assert_equal (Regularizer.concatenate_lists lst1 list_negatives)
    [ (1.5, -1.0); (3.2, -2.0); (4.3, -3.0)]
    ); 
  ( "concatenate_lists test (negative x negative)" >:: fun _ ->
    assert_equal (Regularizer.concatenate_lists list_negatives list_negatives)
    [ (-1.0, -1.0); (-2.0, -2.0); (-3.0, -3.0)]
    ); 
]

let test_regularizer = "test suite for regularizer.ml" >::: List.flatten [
  test_open; 
  test_ones; 
  test_issues_weights; 
